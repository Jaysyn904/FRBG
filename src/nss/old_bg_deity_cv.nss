...
{  
	...
	{  
		...
		{  
			...
			{  
                SetHeader("Select your patron deity:");
			...
			{    
				int nSelected = GetLocalInt(oPC, "deity_selected");  // Use stored choice  
				SetHeader(GetDeityText(oPC, nSelected));    
				AddChoice("Yes!", nSelected, oPC);   // Pass the selected deity as value    
				AddChoice("No...", -1, oPC);         // Negative value signals decline    
		...
		{  
			...
			if (nStage == STAGE_LIST)  
			{  
				// Store selected deity and move to confirmation  
				SetLocalInt(oPC, "deity_selected", nChoice);  
				SetStage(STAGE_CONFIRM, oPC);  
			}  
			else if (nStage == STAGE_CONFIRM)  
			{  
				if (nChoice >= 0) // "Yes!" - grant the deity  
				{  
					string sGrant;  
					switch (GetLocalInt(oPC, "deity_selected"))    
					{    
						case 1:  
						{  

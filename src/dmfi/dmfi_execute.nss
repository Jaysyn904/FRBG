...
// added inclusion for webhook support - Saadow 8/15/24
...
void DoDiceBagFunction(int iDice, object oUser, int iDMOverride = 0)
{
    ...
    case 91: iNum = 1; iSide = 20; sAbility="Diplomacy Check, "; iMod = GetSkillRank(SKILL_PERSUADE, oUser); break;
    case 92: iNum = 1; iSide = 20; iTrain = 1; sAbility="Sleight of Hand Check, "; iMod = GetSkillRank(SKILL_PICK_POCKET, oUser); break;
...
void DoXPFunction(int iXP, object oUser)
{
    ...
    if (iParty==1)
    {
        ...
        // This is a test to experiment making a string accumulator that will gather the names of all party members and spit them out in one webhook at the end - Saadow 8/15/24
        oPartyMember=GetFirstFactionMember(oTarget, TRUE);
        string SaadPartyAccumulator = GetName(oPartyMember);
        while (GetIsObjectValid(oPartyMember))
        {
            ...
            oPartyMember = GetNextFactionMember(oTarget, TRUE);
            SaadPartyAccumulator = SaadPartyAccumulator + ", " + GetName(oPartyMember);
        }
        // SendMessageToAllDMs("The entire party was granted "+ IntToString(iReward)+ " XP.");
        NWNX_WebHook_SendWebHookHTTPS("discordapp.com", WEBHOOK_CHAT_CHANNEL, ("A party consisting of " + SaadPartyAccumulator +" received an experience reward of "+ IntToString(iReward)+ " XP."), GetName(oUser));
    }
    ...
    else  //single player code
    {
        ...
        NWNX_WebHook_SendWebHookHTTPS("discordapp.com", WEBHOOK_CHAT_CHANNEL, (GetName(oTarget) +" received a "+GetLocalString(oUser, "BonusType")+ " experience reward of "+ IntToString(iReward)+ "."), GetName(oUser));
    }
    return;
}
